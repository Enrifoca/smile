...
| Connector source memory | `sources/<connector>/<scope>/` | Write outcomes in monitored scopes; loaded automatically when relevant |

## Learned note budget

Recent learned notes load verbatim into the prompt (newest first, char budget in `constants.ts`). When total learned text exceeds the rollup threshold, older notes are condensed into **Archived Rollup** — still kept in full on disk.
