...
Native modules such as `better-sqlite3` are unpacked from the asar archive automatically (`asarUnpack: ["**/*.node"]`). `npm install` also runs `electron-builder install-app-deps` so native modules are rebuilt for the installed Electron version automatically. If you ever need to rebuild manually:
